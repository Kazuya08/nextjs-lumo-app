                ...
                <div className={isDarkMode ? "dark" : ""}>
                    <div className="w-full h-[100dvh] flex items-center justify-center bg-background text-foreground p-0 sm:p-4 md:p-6 overflow-hidden">
                        <div className="w-full h-full lg:w-[1100px] lg:h-[850px] lg:max-w-[1200px] flex flex-col lg:flex-row bg-card text-foreground rounded-none sm:rounded-2xl lg:rounded-3xl overflow-hidden shadow-none lg:shadow-2xl border-0 lg:border border-border relative">
                            ...
                            {/* ========================================================= */}
                            <div className={`hidden lg:flex relative lg:w-[40%] xl:w-[50%] h-full flex-col justify-between p-8 xl:p-12 overflow-hidden bg-zinc-950 shrink-0 ${isRegister ? "lg:order-2" : "lg:order-1"}`}>
                                <div className="absolute inset-0 z-0">
                                    <div className={`absolute inset-0 bg-gradient-to-t ${isRegister ? "lg:bg-gradient-to-l" : "lg:bg-gradient-to-r"} from-zinc-950 via-zinc-950/80 to-transparent z-20`} />
                                    ...
                                    <img
                                        ...
                                        className={`absolute inset-0 w-full h-full object-cover object-center transition-opacity duration-500 ${!isRegister ? "opacity-100 z-10" : "opacity-0 z-0"}`}
                                    />
                                    <img
                                        src="https://i.pinimg.com/736x/c7/63/64/c7636409decf56ab820fe24f3a5a62d8.jpg" 
                                        alt="Nova Aventura"
                                ...
                                <div className="relative z-40 mt-4 mb-auto py-2 max-w-xl">
                                    ...
                                    <div className="grid grid-cols-3 gap-3 pt-3 border-t border-zinc-800/80">
                                        <div className="flex items-center gap-2.5 text-zinc-300 text-xs xl:text-sm font-medium">
                                            ...
                                            <span>{isRegister ? "Registre sua história" : "Jogos finalizados"}</span>
                                        </div>
                                        <div className="flex items-center gap-2.5 text-zinc-300 text-xs xl:text-sm font-medium">
                                            <span className="p-2 rounded-lg bg-primary/10 text-primary border border-primary/20 shrink-0">
                                                <Sparkles className="w-4 h-4" />
                                            </span>
                                            <span>{isRegister ? "Evolua com XP" : "Conquiste XP"}</span>
                                        </div>
                                        <div className="flex items-center gap-2.5 text-zinc-300 text-xs xl:text-sm font-medium">
                                            <span className="p-2 rounded-lg bg-primary/10 text-primary border border-primary/20 shrink-0">
                                                <Gamepad2 className="w-4 h-4" />
                                            </span>
                                            <span>{isRegister ? "Descubra novos jogos" : "Novas aventuras"}</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {/* ========================================================= */}
                            <div className={`w-full lg:w-[50%] xl:w-[60%] h-full flex flex-col justify-between p-4 sm:p-6 lg:p-8 bg-card relative overflow-hidden ${isRegister ? "lg:order-1" : "lg:order-2"}`}>

                                <div className="flex items-center justify-between w-full shrink-0 h-9 mb-2">
                                   <div className="relative z-30 flex items-center justify-between">
                                    <Link href="/" className="inline-flex items-center gap-2 text-xs font-medium text-zinc-300 hover:text-white bg-zinc-900/85 border border-zinc-800 px-3 py-2 rounded-lg transition-colors group ml-auto">
                                    ...
                                    >
                                        {isDarkMode ? <Sun className="w-3.5 h-3.5 text-primary" /> : <Moon className="w-3.5 h-3.5" />}
                                        <span className="hidden sm:inline">{isDarkMode ? "Modo Claro" : "Tema Escuro"}</span>
                                        <span className="sm:hidden">{isDarkMode ? "Claro" : "Escuro"}</span>
                                    </button>
                                </div>
